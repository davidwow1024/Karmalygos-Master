...
public:
    ...
    static bool HandleModifyHPCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (Player* player = target->ToPlayer())
        {
            ...
            if (handler->needReportToTarget(player))
                ChatHandler(player->GetSession()).PSendSysMessage(LANG_YOURS_HP_CHANGED, handler->GetNameLink().c_str(), hp, hpm);
        }
        else
        {
            handler->PSendSysMessage(LANG_YOU_CHANGE_HP, target->GetName().c_str(), hp, hpm);
        }
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetTrinityString(ReputationRankStrIndex(ReputationRank(r)));
                if (rank.empty())
                    continue;
    ...
    static bool HandleModifyStandStateCommand(ChatHandler* handler, const char* args)
    {
        ...
        handler->GetSession()->GetPlayer()->HandleEmoteStateCommand(anim_id);
...
//demorph player or unit
    ...
    static bool HandleModifyCurrencyCommand(ChatHandler* handler, const char* args)
    {
        ...
        Player* target = handler->getSelectedPlayer();
        if (!target)
            return handler->SendError(LANG_PLAYER_NOT_FOUND);

        Tokenizer toks{ args, ' ' };
        if (toks.size() < 2)
            return false;

        uint32 currencyId = atol(toks[0]);
        const CurrencyTypesEntry* currencyType =  sCurrencyTypesStore.LookupEntry(currencyId);
        if (!currencyType)
            return false;

        uint32 amount = atol(toks[1]);
        if (!amount)
            return false;

        bool ignoreCap = false;
        if (toks.size() > 2)
            ignoreCap = atol(toks[2]) != 0;

        int32 precision = currencyType->Flags & CURRENCY_FLAG_HIGH_PRECISION ? CURRENCY_PRECISION : 1;

        target->ModifyCurrency(currencyId, amount * precision, true, true, ignoreCap);
