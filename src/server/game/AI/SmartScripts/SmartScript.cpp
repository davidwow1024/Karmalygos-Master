...
SmartScript::SmartScript()
{
    ...
    mEventPhase = 0;
    mPathId = 0;
    mTargetStorage = new GuidListMap();
    mTextTimer = 0;
    mLastTextID = 0;
    mUseTextTimer = false;
    mTalkerEntry = 0;
    ...
    switch (e.GetActionType())
    {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.cast.spell))
                {
                    if (me)
                    {
                        ...
                        if (e.action.cast.flags & SMARTCAST_FORCE_TARGET_SELF)
                            (*itr)->ToUnit()->CastSpell((*itr)->ToUnit(), e.action.cast.spell, (e.action.cast.flags & SMARTCAST_TRIGGERED));
                        else if (e.action.cast.flags & SMARTCAST_DEST)
                            (*itr)->ToUnit()->CastSpell(e.target.x, e.target.y, e.target.z, e.action.cast.spell, (e.action.cast.flags & SMARTCAST_TRIGGERED));
                        else
                    ...
                    TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST:: Creature %u casts spell %u on target %u with castflags %u",
                        me ? me->GetGUIDLow() : go->GetGUIDLow(), e.action.cast.spell, (*itr)->GetGUIDLow(), e.action.cast.flags);
                }
                else
        ...
        case SMART_ACTION_SET_EMOTE_STATE:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    (*itr)->ToUnit()->HandleEmoteStateCommand(e.action.emote.emote);
        ...
        case SMART_ACTION_CALL_KILLEDMONSTER:
        {
            ...
            else // Specific target type
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    if (IsPlayer(*itr))
                    {
                        (*itr)->ToPlayer()->KilledMonsterCredit(e.action.killedMonster.creature, 0,
                            e.action.killedMonster.countMax ? urand(e.action.killedMonster.countMin, e.action.killedMonster.countMax) : (e.action.killedMonster.countMin ? e.action.killedMonster.countMin : 1));
                        TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: Player %u, Killcredit: %u",
                        ...
                        if (Vehicle* vehicle = (*itr)->ToUnit()->GetVehicleKit())
                            for (SeatMap::iterator itr = vehicle->Seats.begin(); itr != vehicle->Seats.end(); ++itr)
                                if (Player* player = ObjectAccessor::FindPlayer(itr->second.Passenger.Guid))
                                    player->KilledMonsterCredit(e.action.killedMonster.creature, 0,
                                        e.action.killedMonster.countMax ? urand(e.action.killedMonster.countMin, e.action.killedMonster.countMax) : (e.action.killedMonster.countMin ? e.action.killedMonster.countMin : 1));
                }
        ...
        case SMART_ACTION_SET_ACTIVE:
        {
            if (WorldObject* baseObj = GetBaseObject())
                baseObj->setActive(e.action.active.state, ActiveFlags::SAI);
            break;
        }
        ...
        case SMART_ACTION_PLAY_ANIMKIT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    ...
                    TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_PLAY_ANIMKIT: target: %s (" UI64FMTD "), AnimKit: %u, Type: %u",
                        (*itr)->GetName().c_str(), (*itr)->GetGUID(), e.action.animKit.animKit, e.action.animKit.type);
                }
            }
        ...
        case SMART_ACTION_PLAY_SCENE_ID:
        {
            ...
            bool apply = e.action.scene.apply != 0;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (apply)
                    (*itr)->ToPlayer()->GetSceneMgr().PlayScene(sceneTemplate->SceneId);
                else
                    (*itr)->ToPlayer()->GetSceneMgr().CancelSceneByPackageId(sceneTemplate->ScenePackageId);
            }
        ...
        case SMART_ACTION_PLAY_SCENE_PACKAGE:
        {
            ...
            uint32 packageID = e.action.scene.sceneId;
            if (!packageID)
                break;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (apply)
                    (*itr)->ToPlayer()->GetSceneMgr().PlaySceneByPackageId(packageID);
                else
                    (*itr)->ToPlayer()->GetSceneMgr().CancelSceneByPackageId(packageID);
            }
        ...
        case SMART_ACTION_OVERRIDE_INHABIT_TYPE:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    (*itr)->ToCreature()->OverrideInhabitType(InhabitTypeValues(e.action.inhabbitType.type));
                    (*itr)->ToCreature()->UpdateMovementFlags();
                }
            }
        ...
        case SMART_ACTION_STOP_FOLLOW:
        {
            ...
            CAST_AI(SmartAI, me->AI())->SetUnfollow();
            break;
        }
        case SMART_ACTION_SPELL_VISUAL_KIT:
        {
            ...
            me->SendPlaySpellVisualKit(e.action.raw.param1, 0, 0);
            break;
        }
        case SMART_ACTION_CAST_RANDOM_SPELL:
        {
            ...
            uint32 actions[SMART_ACTION_PARAM_COUNT - 1];
            actions[0] = e.action.castRandomSpell.spell1;
            actions[1] = e.action.castRandomSpell.spell2;
            actions[2] = e.action.castRandomSpell.spell3;
            actions[3] = e.action.castRandomSpell.spell4;
            actions[4] = e.action.castRandomSpell.spell5;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (!(e.action.castRandomSpell.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(id))
                {
                    if (e.action.castRandomSpell.flags & SMARTCAST_INTERRUPT_PREVIOUS)
                        me->InterruptNonMeleeSpells(false);

                    if (e.action.castRandomSpell.flags & SMARTCAST_COMBAT_MOVE)
                    {
                        // If cast flag SMARTCAST_COMBAT_MOVE is set combat movement will not be allowed
                        // unless target is outside spell range, out of mana, or LOS.
                    ...
                    if (e.action.castRandomSpell.flags & SMARTCAST_FORCE_TARGET_SELF)
                        (*itr)->ToUnit()->CastSpell((*itr)->ToUnit(), id, (e.action.castRandomSpell.flags & SMARTCAST_TRIGGERED));
                    else if (e.action.castRandomSpell.flags & SMARTCAST_DEST)
                        (*itr)->ToUnit()->CastSpell(e.target.x, e.target.y, e.target.z, id, (e.action.castRandomSpell.flags & SMARTCAST_TRIGGERED));
                    else
                        me->CastSpell((*itr)->ToUnit(), id, (e.action.castRandomSpell.flags & SMARTCAST_TRIGGERED));

                    TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST_RANDOM_SPELL:: Creature %u casts spell %u on target %u with castflags %u",
                        me->GetGUIDLow(), id, (*itr)->GetGUIDLow(), e.action.castRandomSpell.flags);
                }
                else
                    TC_LOG_DEBUG("scripts.ai", "Spell %u not casted because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*itr)->GetGUID(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
            }
        ...
        case SMART_TARGET_STORED:
        {
            ...
            if (itr != mTargetStorage->end())
                for (GuidList::const_iterator itr2 = itr->second.begin(); itr2 != itr->second.end(); ++itr2)
                    if (WorldObject* obj = ObjectAccessor::GetWorldObject(*baseObject, *itr2))
                        l->push_back(obj);
            return l;
        }
...
void SmartScript::UpdateTimer(SmartScriptHolder& e, uint32 const diff)
{
    ...
    if (e.timer < diff)
    {
        ...
        if (e.GetActionType() == SMART_ACTION_CAST || e.GetActionType() == SMART_ACTION_CAST_RANDOM_SPELL)
        {
            if (!(e.action.cast.flags & SMARTCAST_INTERRUPT_PREVIOUS))
            {
